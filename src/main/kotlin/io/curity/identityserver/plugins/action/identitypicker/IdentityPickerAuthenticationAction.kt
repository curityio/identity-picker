...
class IdentityPickerAuthenticationAction(private val config: IdentityPickerAuthenticationActionConfig) :
    ...
    override fun apply(context: AuthenticationActionContext): AuthenticationActionResult {
        ...
        val authenticationAttributes = context.authenticationAttributes
        ...
        val identityList = authenticationAttributes
            .subjectAttributes[config.identityListAttribute()]?.attributeValue as? ListAttributeValue

        logger.debug("Received list of identities: $identityList")

        if (identityList == null) {
            logger.info(
                "User with with subject '${authenticationAttributes.subject} did not have a list of " +
